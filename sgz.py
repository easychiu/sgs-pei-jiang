...
class General:
    def __init__(self, raw):
        ...
        apt = raw.get("affinity", {})
        best = max(apt, key=lambda k: APT_RANK.get(apt.get(k)), default="騎")
        self.troop = best if best in COUNTER or best == "器" else "騎"
        mult = APT_PCT[apt.get(best)]
        st = raw.get("stats", {})
        self.force = st.get("武力", 80) * mult
        self.intel = st.get("智力", 80) * mult
        self.command = st.get("統率", 90) * mult
        self.speed = st.get("速度", 70) * mult
...
def score(team):
    ...
    attr = sum(max(x.force, x.intel) + x.command + x.speed for x in g)
    kinds = set()
    for x in g:
        if x.tactic:
            kinds.add(x.tactic["type"])
            kinds |= {e["k"] for e in x.tactic["effects"]}
    synergy = len(kinds) * 25
    same_troop = 50 if len({x.troop for x in g}) == 1 else 0
    same_fac = 40 if len({x.faction for x in g}) == 1 else 0
    return round(attr / 3 + synergy + same_troop + same_fac)
...
def recommend(pool=None, k=3, top=8):
    ...
    return [(list(c), score(c)) for c in ranked[:top]]
...
class Unit:
    def __init__(self, g):
        self.g, self.troop, self.stun = g, START_TROOP, 0
...
def damage(src, dst, coef, kind, src_troop=None):
    ...
    base *= counter_mult(src.g.troop, dst.g.troop)
...
def fight(teamA, teamB):
    A = [Unit(POOL[n]) for n in teamA]
    B = [Unit(POOL[n]) for n in teamB]
    setA = set(map(id, A))
...
def simulate(teamA, teamB, n=3000):
    w = {"A": 0, "B": 0}
    rs = 0
    for _ in range(n):
        winner, r = fight(teamA, teamB)
        w[winner] += 1
        rs += r
...
def demo():
    ...
    lb = POOL["呂布"]
    assert lb.force > 250, f"呂布高武, {lb.force:.0f}"
    u = Unit(lb)
    ...
    # 結算傷害(猛毒): 受擊累積層數
    caster, tgt = Unit(POOL["諸葛亮"]), Unit(lb)
    ...
    u2 = Unit(lb)
    u2.adds.append(["extra", 1.0, 9])
    assert u2.addbonus("extra") == 1.0, "連擊預算累計"
    # 疊加增益: 層數推高增傷
    u3 = Unit(lb)
...
def info(name):
    ...
    if not g:
        ...
        return
    print(f"{g.name}  {g.faction} {g.troop}兵  "
          f"武{g.force:.0f} 智{g.intel:.0f} 統{g.command:.0f} 速{g.speed:.0f}")
...
if __name__ == "__main__":
    ...
    if cmd == "sim" and len(sys.argv) >= 4:           # sim "A,B,C" "D,E,F"
        a, b = _team(sys.argv[2]), _team(sys.argv[3])
        print(f"{'/'.join(a)}  vs  {'/'.join(b)}")
        print(" ", simulate(a, b))
    elif cmd == "rec":                                # rec [勢力]
        ...
        for team, sc in recommend(pool):
            print(f"  {sc:>4}  {' / '.join(team)}")
