...
function openTactic(side, i, k) {                              // 選武將卡第 k 個戰法欄(單選, 點即替換)
  ...
  const draw = q => {
    grid.innerHTML = TACTIC_NAMES.filter(t => !q || t.includes(q)).slice(0, 150)
      .map(t => `<div class="pick${t === cur ? " on" : ""}" data-t="${t}">${t}</div>`).join("");
    grid.querySelectorAll(".pick").forEach(p => p.onclick = () => set(p.dataset.t));
  };
  draw("");
  box.querySelector("#tacSearch").oninput = e => draw(e.target.value.trim());
