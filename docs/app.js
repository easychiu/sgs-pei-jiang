...
function facBadge(f) { return `<span class="fac" style="background:${FACBG[f] || "#777"}">${f}</span>`; }
...
async function load() {
  ...
  initTabs(); renderSlots("A"); renderSlots("B"); renderDex();
...
function renderSlots(side) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    d.className = "slot";
    d.innerHTML = g
      ? `${facBadge(g.faction)}<div><div class="nm">${n}</div><div class="sub">${g.troop}兵 · 武${g.force | 0} 智${g.intel | 0} 統${g.command | 0} 速${g.speed | 0}</div></div>`
...
function runSim() {
  ...
  const r = SGZ.simulate(POOL, A, B, 3000);
  ...
  res.innerHTML = `
    ...
    <div>我方 <b class="gold">${pct(r.winA)}%</b> 勝　·　敵方 <b class="gold">${pct(r.winB)}%</b> 勝　·　平均 ${r.rounds} 回合</div>
...
function runRec() {
  ...
  $("#recList").innerHTML = list.map(([team, sc]) =>
    `<li data-team='${JSON.stringify(team)}'><span>${team.join("　／　")}</span><span class="sc">${sc}</span></li>`).join("");
  document.querySelectorAll("#recList li").forEach(li => li.onclick = () => {
    teams.A = [...JSON.parse(li.dataset.team)]; renderSlots("A");
...
function renderDex() {
  ...
  $("#dexGrid").innerHTML = names.map(n => {
    ...
    return `<div class="gcard" data-n="${n}">
      ...
      <div class="sub">${g.faction} · ${g.troop}兵</div></div></div>`;
  }).join("");
...
function showDetail(n) {
  ...
  box.innerHTML = `<div class="detail">
    ...
    <div class="meta"><h2 class="gold" style="margin:0">${facBadge(g.faction)} ${n}
      ...
      <div class="stat-row"><span><b>武</b> ${g.force | 0}</span><span><b>智</b> ${g.intel | 0}</span>
      <span><b>統</b> ${g.command | 0}</span><span><b>速</b> ${g.speed | 0}</span></div>
      <div class="sub">主用兵種：${g.troop}（適性已計入面板）</div>
