...
async function load() {
  ...
  const [g, t, bs, bo, eq, sm, to, tc] = await Promise.all([
...
function openTactic(side, i, k) {                              // 選武將卡第 k 個戰法欄(單選, 點即替換)
  ...
  const ownNative = g ? g.tacticName : null;                  // 篩掉本將自帶
  ...
  const draw = () => {
    ...
    grid.innerHTML = TACTIC_NAMES.filter(t => t !== ownNative && (cat === "全部" || tacCat(t) === cat) && (!q || t.includes(q)))
      .slice(0, 150).map(t => {
        const lk = lockedCats.includes(tacCat(t)) && t !== cur;   // 該分類同隊已佔
        return `<div class="pick${t === cur ? " on" : ""}${lk ? " locked" : ""}" data-t="${t}"${lk ? ` title="同隊已有${tacCat(t)}戰法"` : ""}>${t}</div>`;
      }).join("");
